...
import com.funkycorgi.vulpecula.entry.common.repo.DbEntryResponseErr
...
class EntryBizRepoDeleteTest {
    private val command = EntryCommand.DELETE
    ...
    private val repo = EntryRepositoryMock(
        ...
        invokeDeleteEntry = {
            if (it.id == entryFromRepo.id)
                DbEntryResponseOk(entryFromRepo)
            else DbEntryResponseErr()
        }
    )
    ...
    fun repoDeleteSuccessTest() = runTest {
        ...
        assertEquals(entryFromRepo.id, ctx.entryResponse.id)
        assertEquals(entryFromRepo.movieId, ctx.entryResponse.movieId)
        assertEquals(entryFromRepo.rating, ctx.entryResponse.rating)
        assertEquals(entryFromRepo.comment, ctx.entryResponse.comment)
    }
