...
fun ICorChainDsl<EntryContext>.stubValidationBadViewingDate(title: String) = worker {
    this.title = title
    on { stubCase == EntryStubs.BAD_VIEWING_DATE && state == EntryState.RUNNING }
    handle {
        fail(
            EntryError(
                group = "validation",
                code = "validation-viewing-date",
                field = "viewingDate",
                message = "Wrong viewingDate field"
            )
        )
    }
}
