...
fun ICorChainDsl<EntryContext>.stubValidationBadMovieId(title: String) = worker {
    this.title = title
    on { stubCase == EntryStubs.BAD_MOVIE_ID && state == EntryState.RUNNING }
    handle {
        fail(
            EntryError(
                group = "validation",
                code = "validation-movie-id",
                field = "movieId",
                message = "Wrong movieId field"
            )
        )
    }
}
