...
fun ICorChainDsl<EntryContext>.repoSearch(title: String) = worker {
    this.title = title
    on { state == EntryState.RUNNING && entryRepo != IRepoEntry.NONE }
    handle {
        when (val result = entryRepo.searchEntry(DbEntryFilterRequest(entryFilterValidated))) {
            is DbEntriesResponseOk -> entriesRepoDone = result.data.toMutableList()
            is DbEntriesResponseErr -> fail(result.errors)
        }
    }
}
